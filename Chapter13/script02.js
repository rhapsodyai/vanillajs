  ...
  if(xhr.readyState == 4) {
    if(xhr.status== 200) {
      ...
      for(var i=0; i<allImages.length; i++) {
        ...
        document.getElementById("pictureBear").innerHTML += theText;
      }
    }
    else {
      alert("Therewasa problem with the request " + xhr.status);
    }
  }

  function getPixVal(inVal) {
    return (inVal.textContent) ? inVal.textContent : inVal.txt;
  }
}
